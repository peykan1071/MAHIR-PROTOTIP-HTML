...
def outcome_key_for(question: dict[str, Any]) -> str:
    ...
    return " | ".join(
        value for value in (question.get("outcomeTheme"), question.get("outcomeCode")) if value
    ) or f"Soru {question.get('number')}"
...
def build_ced_from_payload(
    ...
    ced_questions = [
        CEDQuestion(
            ...
            learning_outcome_ids=[outcome_key_for(question)],
        )
